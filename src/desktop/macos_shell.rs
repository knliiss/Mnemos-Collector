...
pub fn run(context: DesktopLaunchContext, runtime: Handle) -> Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(move |creation_context| {
            ...
            Ok(Box::new(MacDesktop::new(context, runtime)))
        }),
    )
...
pub fn show_fatal_error(message: &str) {
    macos_native::show_error(message);
}
...
impl MacDesktop {
    ...
    fn begin_activation(&mut self) {
        ...
        runtime.spawn(async move {
            ...
            let result = if current_installation {
                ...
                provision_install_and_launch(token.as_str(), &device_name)
                    .await
    ...
    fn poll_activation(&mut self, context: &egui::Context) {
        ...
        match result {
            ...
            Ok(ActivationOutcome::InstalledAndLaunched) => {
                diagnostics::info(
                    "provisioning",
                    "Provisioned Collector installed and launched from stable location",
                );
    ...
    fn draw_header(&mut self, ui: &mut egui::Ui, context: &egui::Context, layout: Layout) {
        ...
        draw_card(painter, icon, SURFACE, LINE, 18.0);
        draw_mascot(painter, icon.shrink(2.0));

        draw_text(painter, Pos2::new(78.0, 14.0), "MNEMOS", 13.0, ACCENT, true);
        draw_text(painter, Pos2::new(78.0, 34.0), "Collector", 21.0, TEXT, true);
        ...
        draw_window_button(painter, layout.window_close, "×", true, close.hovered());
    ...
    fn draw_hero(&self, ui: &mut egui::Ui, runtime: &RuntimeSnapshot, hero: Rect) {
        ...
        draw_card(painter, hero, SURFACE, LINE, CARD_RADIUS);
        painter.rect_filled(
            Rect::from_min_max(
        ...
        draw_mascot(painter, mascot);
        draw_status_tiles(painter, runtime, hero);
    }

    fn draw_activation(&mut self, ui: &mut egui::Ui, activation: Rect, layout: Layout) {
        let painter = ui.painter();

        draw_card(painter, activation, SURFACE, LINE, CARD_RADIUS);
        draw_text(
            painter,
        ...
        draw_input_background(painter, layout.token_field);
        draw_input_background(painter, layout.device_field);
    ...
    fn draw_journal(&self, ui: &mut egui::Ui, context: &egui::Context, layout: Layout) {
        let painter = ui.painter();

        draw_card(painter, layout.logs_card, SURFACE, LINE, CARD_RADIUS);
        draw_text(
            painter,
            Pos2::new(layout.logs_card.left() + 16.0, layout.logs_card.top() + 13.0),
            "Журнал",
            21.0,
            TEXT,
            true,
        ...
        draw_secondary_button(painter, layout.copy_logs, "Копировать всё", copy.hovered());
        ...
        draw_card(painter, layout.logs_view, LOG_SURFACE, LINE, 18.0);
        draw_log_text(painter, layout.logs_view, &diagnostics::recent_text());
    }
...
impl Layout {
    fn new(provisioned: bool) -> Self {
        ...
        let logs_top = activation
            .map_or(hero.bottom() + SECTION_GAP, |card| card.bottom() + SECTION_GAP);
...
fn draw_window_button(
    ...
    let fill = if hovered {
        if destructive {
            Color32::from_rgb(0x31, 0x16, 0x18)
        } else {
            SURFACE_RAISED
        }
    } else {
    ...
    painter.text(
        ...
        if destructive && hovered { DANGER } else { TEXT_SECONDARY },
    );
}
...
fn draw_status_tiles(painter: &egui::Painter, runtime: &RuntimeSnapshot, hero: Rect) {
    ...
    let game = Rect::from_min_size(Pos2::new(left, top), Vec2::new(tile_width, STATUS_TILE_HEIGHT));
    ...
    draw_status_tile(
        ...
        game,
        "ИГРА",
        if runtime.cristalix_running { "Cristalix" } else { "Ожидание" },
        if runtime.cristalix_running { POSITIVE } else { TEXT_MUTED },
    );
    draw_status_tile(
        painter,
        mode,
        ...
        if is_master_sword(runtime.game_mode.as_str()) { ACCENT } else { AMBER },
    );
    draw_status_tile(
        painter,
        mnemos,
        "MNEMOS",
        if runtime.realtime_connected { "Подключён" } else { "Нет связи" },
        if runtime.realtime_connected { POSITIVE } else { DANGER },
    );
}
...
fn draw_status_tile(
    ...
    painter.circle_filled(Pos2::new(rect.left() + 11.0, rect.top() + 10.0), 4.0, status_color);
    draw_text(
        painter,
...
fn draw_primary_button(
    ...
    let text = if hovered && !disabled { BACKGROUND } else { TEXT };
...
fn draw_toggle(
    ...
    draw_card(
        ...
        if enabled { ACCENT } else if hovered { LINE_STRONG } else { LINE },
        15.0,
    );
    ...
    painter.text(
        ...
        label,
        FontId::new(13.0, FontFamily::Proportional),
        if enabled || hovered { TEXT } else { TEXT_SECONDARY },
    );
}

fn draw_log_text(painter: &egui::Painter, rect: Rect, text: &str) {
    ...
    let visible_lines = (text_rect.height() / LOG_LINE_HEIGHT).floor().max(1.0) as usize;
...
async fn provision_install_and_launch(token: &str, device_name: &str) -> Result<()> {
    ProvisioningClient::new()?
        .provision(token, device_name)
        .await
        .context("не удалось активировать Collector")?;

    tokio::task::spawn_blocking(Installation::migrate_existing_and_launch)
        .await
